...
const jenkins = {
    ...
    deploy: (jobName, parameters) => {
        let fromConfig = JSON.parse(fs.readFileSync(`config/aws/${parameters.from_env}.json`));
        let toConfig = JSON.parse(fs.readFileSync(`config/aws/${parameters.to_env}.json`));
        ...
        parameters.from_stage = fromConfig.stage;
        ...
        parameters.to_stage = toConfig.stage;
        parameters.certificate_arn = toConfig.sslCertificateArn;
