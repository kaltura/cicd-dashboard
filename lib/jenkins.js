...
const jenkins = {
    ...
    init: (flowData, logger, config) => {
        jenkins.logger = logger;
        jenkins.config = config;
        jenkins.flowData = flowData;
        jenkins.client = jenkinsApi.init(config.jenkinsUrl),
...
module.exports = (flowData, logger, config) => {
    jenkins.init(flowData, logger, config)
    return jenkins;
};
