...
class EnvDeploy {
    constructor(env, src, bucket, additionalJobs) {
            ...
            var stepInto = () => {
                ...
                step.apply(This, [firstRun]).then(({type, msg}) => {
                    ...
                    if(This.items.length) {
                        ...
                        if(This.additionalJobs && This.additionalJobs.length) {
                            This.additionalJobs.forEach(jobName => {
                                deploy.jenkins.build(jobName).then(() => {}, err => console.log(`Failed to build Jenkins job [${jobName}]`, err));
                            });
                        }
    ...
    reposPushed(firstRun) {
        ...
        var push = () => {
            ...
            windowsRepos
            .map(repoToParams)
            .map(params => deploy.jenkins.deploy('Push-Docker-Windows', This.src, This.env, params));
        ...
        return new Promise((resolve, reject) => {            
            Promise
                ...
                .then(() => resolve({type: 'repos', msg: 'Required images deployed'}), (missing) => {
                    ...
                    reject({type: 'repos', msg: `Required images are not deployed yet (${missing})`});
                });
        });
    }
            ...
            This.autoscaling.describeAutoScalingGroups({AutoScalingGroupNames: ['swarm-' + This.env + '-manager-linux']}, (err, data) => {
                ...
                if(data.AutoScalingGroups && data.AutoScalingGroups.length) {
                    ...
                    if(autoScalingGroup.DesiredCapacity == 0) {
                        This.autoscaling.setDesiredCapacity({AutoScalingGroupName: 'swarm-' + This.env + '-manager-windows', DesiredCapacity: 1}, (err, data) => {
                            if(err) {
                                deploy.logger.error('Failed to scale up windows managers auto-scailing-group', err);
                            }
                            else {
                                deploy.logger.log('Scaled up windows managers auto-scailing-group');
                            }
                        });
                        return This.autoscaling.setDesiredCapacity({AutoScalingGroupName: 'swarm-' + This.env + '-manager-linux', DesiredCapacity: 1}, (err, data) => {
                            if(err) {
                                return reject({type: 'master', msg: 'Failed to scale up auto-scailing-group'});
                            }
                            reject({type: 'master', msg: 'Scaled up auto-scailing-group'});
                        });
                    }
                    else if(autoScalingGroup.Instances && autoScalingGroup.Instances.length) {
                        return resolve({type: 'master', msg: 'Started'});
                    }                    
                    reject({type: 'master', msg: 'Not started yet'});
                }
                reject({type: 'master', msg: 'Auto-Scaling-Group not found'});
            });
        });
    }
    ...
    clusterStarted() {
        var This = this;
        return new Promise((resolve, reject) => {
            This.s3.getObject({Bucket: This.bucket, Key: 'manager-token'}, (err, data) => {
        ...
        return new Promise((resolve, reject) => {
            This.autoscaling.describeAutoScalingGroups({AutoScalingGroupNames: ['swarm-' + This.env + '-manager-linux']}, (err, data) => {
                ...
                if(plannedCapacity > autoScalingGroup.DesiredCapacity) {
                    return This.autoscaling.setDesiredCapacity({AutoScalingGroupName: 'swarm-' + This.env + '-manager-linux', DesiredCapacity: autoScalingGroup.DesiredCapacity + 1}, (err, data) => {
                        if(err) {
                            return reject({type: 'managers', msg: 'Failed to scale up'});
                        }
                        reject({type: 'managers', msg: 'Scaled up to ' + plannedCapacity, delay: 30000});
                    });
                }
                ...
                for(var i = 0; i < autoScalingGroups.length; i++) {
                    ...
                    if(plannedCapacity) {
                        ...
                        if(plannedCapacity > asg.DesiredCapacity) {
                            This.autoscaling.setDesiredCapacity({AutoScalingGroupName: asg.AutoScalingGroupName, DesiredCapacity: asg.DesiredCapacity + 1}, (err) => {
                                if(err) {
                                    deploy.logger.error(err);
                                }
                            });
                            return reject({type: 'nodes', msg: 'Scaled up auto-scailing-groups', delay: 30000});
                        }
                    }
                }
                if(plannedInstances > runningInstances) {
                    return reject({type: 'nodes', msg: 'Waiting for instances to start (' + runningInstances + '/' + plannedInstances + ')'});
                }
                resolve({type: 'nodes', msg: 'Ready (' + runningInstances + '/' + plannedInstances + ')'});
            });
        });
    }
...
const deploy = {
    ...
    deploy: (src, env, bucket, additionalJobs, shouldDeploy) => {
        let envDeploy = new EnvDeploy(env, src, bucket, additionalJobs);
        envDeploy.run(shouldDeploy).then(() => {
            envDeploy = null;
        });
