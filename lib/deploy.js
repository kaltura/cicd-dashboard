...
class EnvDeploy {
    ...
    startLinuxManagers() {
        var This = this;
        return new Promise((resolve, reject) => {
            This.autoscaling.setDesiredCapacity({AutoScalingGroupName: 'swarm-' + This.env + '-manager-linux', DesiredCapacity: 5}, (err, data) => {
                if(err) {
                    return reject(err);
                }
                deploy.logger.log('Scaling up ' + This.env + ' linux managers');
...
const deploy = {
    ...
    deploy: (src, env) => {
        ...
        envDeploy.masterStarted()
            ...
            .then(() => envDeploy.startLinuxManagers())
