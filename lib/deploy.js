                ...
                step.apply(This, [firstRun]).then(({type, msg}) => {
                    ...
                    if(This.items.length) {
                        ...
                        if(This.additionalJobs && This.additionalJobs.length) {
                            Object.keys(This.additionalJobs).forEach(jobName => {

                                deploy.jenkins.build(jobName, This.additionalJobs[jobName]).then(() => {}, err => console.log(`Failed to build Jenkins job [${jobName}]`, err));
                            });
                        }
                ...
                for(var i = 0; i < autoScalingGroups.length; i++) {
                    ...
                    if(plannedCapacity) {
                        ...
                        if(plannedCapacity > asg.DesiredCapacity) {
                            ...
                            return reject({type: 'nodes', msg: 'Scaled up auto-scailing-groups', delay: 0});
                        }
                    }
                }
