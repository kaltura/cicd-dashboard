...
const api = {
    ...
    updateUser: (req, res, {username, password}) => {
        const email = req.session.user.email;
        return api.model.User.GetByMail(email)
        .then(
            user => api.model.User.Update(user, username, password),
            err => Promise.reject(err)
        );
    },
...
function filterFlow(flow, permissions) {
    ...
    return flow.map(item => {
        if(item.requiredPermissions && item.requiredPermissions.read && !permissions[item.requiredPermissions.read]) {
            return null;
        }
