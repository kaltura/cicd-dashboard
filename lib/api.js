...
const api = {
    ...
    addEnvAsync: (env, flow) => {
        return new Promise((resolve) => {
            setTimeout(() => {
                api.addEnv(env, flow).then(resolve);
            }, 0);
        });
    },
        ...
        return new Promise((resolve) => {
            ...
            for(var i = 0; i < flow.length; i++) {
                if(flow[i].id && flow[i].id == env.parent) {
                    ...
                    if(!flow[i].items.filter(currentEnv => currentEnv.tag === env.tag).length) {                        
                        flow[i].items.push(env);
                    }
                    return Promise.resolve(true);
                }
                else if(typeof flow[i] === 'object') {
                    var keys = Object.keys(flow[i]);
                    for(var j = 0; j < keys.length; j++) {
                        promises.push(api.addEnvAsync(env, flow[i][keys[j]]));
                    }
                }
            }
            Promise.all(promises).then((values => {
                var added = values.filter(value => value).length;
                resolve(added);
            }));
        });
    },
    ...
    env: (env, res) => {
        ...
        api.deployUtils.deploy(env.env.src, env.env.tag).then(() => {
            res.send('ok');
        });
    },
