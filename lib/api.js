...
const api = {
    ...
    addEnv: (env) => {
        ...
        api.envItems[env.tag] = env;
    },

    status: (env, res, {src, tag}) => {
        return api.deployUtils.deploy(src, tag, api.envItems[tag].s3_cluster_bucket, [])
    },
    ...
    env: (env, res) => {
        ...
        api.deployUtils.deploy(env.env.src, env.env.tag, env.env.s3_cluster_bucket, env.additional_jobs, true).then(() => {
            res.send('ok');
        });
    },
