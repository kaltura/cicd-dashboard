...
const api = {
    ...
    loadFlow: () => {        
        ...
        let files = fs.readdirSync(envsDir);
        files.forEach(file => api.addEnv(JSON.parse(fs.readFileSync(`${envsDir}/${file}`))));
    },

    addEnv: (env, flow) => {
        flow = flow || api.flowData;
        if(!Array.isArray(flow)) {
            return false;
        }

        for(var i = 0; i < flow.length; i++) {
            if(flow[i].id && flow[i].id == env.parent) {
                if(!flow[i].items) {
                    flow[i].items = []; 
                }
                flow[i].items.push(env);
                return true;
            }
            else {
                var keys = Object.keys(flow[i]);
                for(var j = 0; j < keys.length; j++) {
                    if(api.addEnv(env, flow[i][keys[j]])) {
                        return true;
                    }
                }
            }
    ...
    env: (env, res) => {
        ...
        var requiredRepos = [
            {from_image: `${env.env.src}-version-deploy:linux-latest`, to_image: `${env.env.tag}-version-deploy:linux-latest`},
            {from_image: `${env.env.src}-tcm-configure:latest`, to_image: `${env.env.tag}-tcm-configure:latest`},
            {from_image: `${env.env.src}-version-updater:linux-latest`, to_image: `${env.env.tag}-version-updater:linux-latest`},
            {from_image: `${env.env.src}-logstash:latest`, to_image: `${env.env.tag}-logstash:latest`},
        ];
        Promise
            .all(requiredRepos.map(params => api.jenkins.deploy('Push-Docker-Linux', env.env.src, env.env.tag, params)))
            .then(() => res.send('ok'));
            
        var windowsRepos = [
            {from_image: `${env.env.src}-version-deploy:windows-latest`, to_image: `${env.env.tag}-version-deploy:windows-latest`},
            {from_image: `${env.env.src}-version-updater:windows-latest`, to_image: `${env.env.tag}-version-updater:windows-latest`},
            {from_image: `${env.env.src}-ecr-login:windows-latest`, to_image: `${env.env.tag}-ecr-login:windows-latest`},
            {from_image: `${env.env.src}-filebeat:latest`, to_image: `${env.env.tag}-filebeat:latest`},
            {from_image: `${env.env.src}-phoenix:latest`, to_image: `${env.env.tag}-phoenix:latest`},
            {from_image: `${env.env.src}-ingest:latest`, to_image: `${env.env.tag}-ingest:latest`},
            {from_image: `${env.env.src}-web-services:latest`, to_image: `${env.env.tag}-web-services:latest`},
            {from_image: `${env.env.src}-tvp-api:latest`, to_image: `${env.env.tag}-tvp-api:latest`},
        ];
        windowsRepos.map(params => api.jenkins.deploy('Push-Docker-Windows', env.env.src, env.env.tag, params));

        var linuxRepos = [
            {from_image: `${env.env.src}-ecr-login:linux-latest`, to_image: `${env.env.tag}-ecr-login:linux-latest`},
            {from_image: `${env.env.src}-tcm:latest`, to_image: `${env.env.tag}-tcm:latest`},
            {from_image: `${env.env.src}-gateway:latest`, to_image: `${env.env.tag}-gateway:latest`},
            {from_image: `${env.env.src}-opc:latest`, to_image: `${env.env.tag}-opc:latest`},
            {from_image: `${env.env.src}-ttv-engine:latest`, to_image: `${env.env.tag}-ttv-engine:latest`},
            {from_image: `${env.env.src}-phoenix-doc:latest`, to_image: `${env.env.tag}-phoenix-doc:latest`},
            {from_image: `${env.env.src}-tcm-ui:latest`, to_image: `${env.env.tag}-tcm-ui:latest`},
            {from_image: `${env.env.src}-reports-uploader:latest`, to_image: `${env.env.tag}-reports-uploader:latest`},
            {from_image: `${env.env.src}-backend-tests:latest`, to_image: `${env.env.tag}-backend-tests:latest`},
            {from_image: `${env.env.src}-elastic-cleaner:latest`, to_image: `${env.env.tag}-elastic-cleaner:latest`},
        ];
        linuxRepos.map(params => api.jenkins.deploy('Push-Docker-Linux', env.env.src, env.env.tag, params));
    },
