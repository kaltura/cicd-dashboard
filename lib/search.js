    ...
    test: (item) => {
        ...
        let suites = (typeof item.message.suites != 'object') ? [] : Object.keys(item.message.suites).map(suiteName => {
            ...
            if(typeof suite.runners === 'object') {
                ...
                value.skiped = runners.map(runner => runner.skiped).reduce((a, b) => a + b);
        ...
        let data = {
            ...
            report: fs.existsSync(__dirname + '/../public/reports/' + item.objectId)
        };
...
const search = {
    ...
    connect: () => {
        ...
        search.elasticSearchSocket.on('error', function open(err) {
            console.log('Elastic-Search web-socket error: ' + err);
        });
        
        search.elasticSearchSocket.on('open', function open() {
            console.log('Elastic-Search web-socket connected');
        });
        
        search.elasticSearchSocket.on('close', function close() {
            console.log('Elastic-Search web-socket disconnected');
    ...
    onNewData: (message) => {
        ...
        switch(data._operation) {
            case 'CREATE':
            case 'INDEX':
                const handler = dataHandler[data._index];
