...
const dataMapper = {
    ...
    test: (item) => {
        ...
        let data = {
            ...
            suites: suites,
            report: fs.existsSync(__dirname + '/../public/reports/' + item.objectId) ? `/reports/${item.objectId}/report/index.html` : false,
            total: 0,
            failed: 0,
            skiped: 0,
            succeed: 0
        ...
        if(suites.length) {
            ...
            data.skiped = suites.map(suite => suite.skiped).reduce((a, b) => a + b);
